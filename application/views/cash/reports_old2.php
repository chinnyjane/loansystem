...
?>
	...
	<!-- TOTAL DISBURSEMENT -->	
			...
			if($banktrans->num_rows() >0){
				...
				$this->table->add_row($count, $bt->bankCode,  $bt->referenceNo, $bt->PN,  $bt->Checkno, $bt->Particulars, $bt->explanation, array("align"=>"right","data"=>$rfplout),array("align"=>"right","data"=>$expout),array("align"=>"right","data"=>$relout), array("align"=>"right","data"=>$ftout),array("align"=>"right","data"=>number_format($out,2)));				
					//$totalcoll[$bt->bankCode][$bt->paymentType][] = $bt->Amount_IN;
					$totalout += $out;
					$totalexp += $exp;
					$totalrel += $rel;
