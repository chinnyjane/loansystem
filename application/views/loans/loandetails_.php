		...
		if($loans['loaninfo']->num_rows() > 0){			
			...
			<table class=" table-condensed table-bordered" width="100%">
				...
				<tbody>
					...
					<tr>
						<td><label>Type of Loan: </label> <?php echo $loaninfo->productName."-".$loaninfo->LoanCode;?></td>
					...
					<tr>
						...
						<?php if($this->auth->perms('Loan Details', $this->auth->user_id(), 3) == true and (strtolower($loanstatus) == 'granted'  or strtolower($loanstatus) == 'current' or strtolower($loanstatus) == 'canceled' or strtolower($loanstatus) == 'closed')){ ?>
	...
	switch ($loanstatus){
			...
			<form action="<?php echo base_url();?>loans/application/action/approve" id="loanappform" method="post">		
				...
				<div class="row form-group">
					...
					<div class="col-md-3">
						...
						<input type="text" value="<?php echo  $loaninfo->Term;?>" name="term" id="term" class="form-control input-sm" readonly>
					</div>
					<div class="col-md-3">
		...
		<?php
			...
			if(empty($loaninfo->PN)){ 
				...
				echo '<button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#assignpn" href="#">Edit Promissory Note (PN)</button>';
...
<div class="tab-content">
	...
	<div class="panel pane-default">
		<?php 
				...
				if($this->auth->perms('Generate Schedule',$this->auth->user_id(),3) == true and strtolower($loanstatus) != 'processing') {  
