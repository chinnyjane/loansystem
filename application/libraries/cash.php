...
class Cash {
	...
	function checkOpenTransbefore($date,$branch){
		$data = array("branchID"=>$branch,
					...
					"isdeleted" => 0 );
		$this->ci->db->where("branchID",$branch);		
		$this->ci->db->where("dateTransaction <",$date);
		$this->ci->db->where("isdeleted", '0');
		$this->ci->db->where("status <>", "verified");
		$this->ci->db->where("status <>", "approved");
		$this->ci->db->select('*');
		$this->ci->db->from('cmctransaction');
		
		//$sql = "select * from cmctransaction where branchID = '".$branch."' and status = 'open' and status <> 'lock' and isdeleted = '0' and dateTransaction < '".$date."' ";
		//echo $sql;
			...
			//$page['banktrans'] = $this->Loansmodel->get_data_from('bankstransactions', $data);		
				...
				if($this->checkOpenTransbefore($page['transdate'],$page['branchid']) == false){
					...
					$data = array("dateVerified"=>$this->ci->auth->localtime(),
								"verifiedBy"=>$this->ci->auth->user_id(),
								"status"=>'verified');
					$this->recompute($_POST['transid']);
					if($this->ci->Loansmodel->update_data('cmctransaction', $where, $data) == true)
						return true;
					else 
						return false;
				}else{
					echo "You still have unverified CMC on previous days. Make sure that these transactions are verified before you proceed.";
					return false;
				}
			}
	}
	...
	function openlocktransaction($transid, $openlock){
		if($openlock == "lock"){
			...
			//$page['banktrans'] = $this->Loansmodel->get_data_from('bankstransactions', $data);		
				...
				//by bank
					...
					echo "You still have unverified transactions. Make sure to verify it before closing current transaction. ";
					return false;
				}
			}else{
		...
		if($this->ci->Loansmodel->update_data('cmctransaction', $where, $data) == true){
			...
			$action = $openlock.": TransID=".$transid.";";
			$this->ci->UserMgmt->audit($module, $action);
			return true;
		}else return false;
	}
					...
					if($this->addDisbursement($transid)==true){
						...
						if(isset($_POST['loanid'])){
							...
							if($loancode == 'E'){
								...
								$startPay = $this->ci->auth->localdate();
								$term = $_POST['term'];
							}
							...
							$this->ci->Loansmodel->update_data("loanapplication", $id, $data);						
							$this->ci->loansetup->update_loanschedule($term, $approve, $startPay, $loanid, $method);
						}
					}
					//echo $status;
					break;
			...
			if($banktrans->num_rows() >0){
				...
				foreach($banktrans->result() as $bt){
					...
					$pos	=0;
						...
						if( $this->ci->auth->perms("reverse", $this->ci->auth->user_id(), 3) == true and $cmcstatus == 'open') {
							$act .= "<a href='".base_url()."cash/daily/reverse/".$transid."/".$bt->BanktransID." 'title='Reverse' data-target='#' data-toggle='reverse'><span class='glyphicon glyphicon-retweet'></span></a>";	
						}
						...
						$this->ci->table->add_row($count, $bt->bankCode.$bcode, $bt->referenceNo, $bt->PN, $bt->Particulars,$bt->transType,$cashin,$checkin, $onlinein, $posin, number_format($in,2), $ver,$act);		
			...
			if($banktrans->num_rows() >0){
				...
				foreach($banktrans->result() as $bt){
					...
					$rfpl =0;
					 ...
					 $this->ci->table->add_row($count, $bt->bankCode.$bcode, $bt->referenceNo,$bt->Checkno,$bt->PN, $bt->Particulars, $bt->explanation, $rfplout, $expout,$relout, $ftout,number_format($out,2), $ver, $act );				
					//$totalcoll[$bt->bankCode][$bt->paymentType][] = $bt->Amount_IN;
				...
				$div .= '</div>';
				return $div;
			}else return $this->ci->table->generate();
		...
		$this->ci->table->set_heading('#', 'JV #', 'Bank Code', 'Particulars','Adjustment Type', 'Amount',' Total Adjustment', 'Verified','Action');			
			if($banktrans->num_rows() >0){
				...
				foreach($banktrans->result() as $bt){
					...
					$this->ci->table->add_row($count, $bt->referenceNo, $bt->bankCode.$bcode, $bt->Particulars, $bt->transType, number_format($amount,2),  number_format($amount,2),$ver, $act);				
					$count++;
					}					
				$div = '<div class="table-responsive">';
				$div .= $this->ci->table->generate();
	...
	function banksByTransID($branchid, $transid, $cmcstatus,$transdate){
		...
		if($banks->num_rows() > 0 ){
			...
			foreach($banks->result() as $bal){
				...
				$this->ci->table->add_row($count, '<a href="'.base_url().'cash/daily/transaction/'.$transid.'/'.$bal->branchBankID.'">'.$bal->bankCode.$bcode.'</a>',$begbal, $totalCol, $totalDis, $totalAdj, $totalEnd, $totalDif, $totalAct, $but);
	...
	function validatecollection(){
		...
		$this->ci->form_validation->set_rules('beginbal', "Beginning Balance", "required|is_numeric");
		$this->ci->form_validation->set_rules('amountwithdrawn', "amount withdrawn", "required|is_numeric");
		if($this->ci->form_validation->run() == false){
			return false;
		}else{
			$begin = $_POST['beginbal'];
			$withdraw = $_POST['amountwithdrawn'];
			$bankbal = $_POST['amountleft'];
			
			$totaldue = 0;
			$this->ci->db->trans_start();
			
			$collectiondata = array("pensionID"=>$_POST['pensionid'],
								"beginbal"=>$begin,
			...
			$this->ci->Loansmodel->addtotable("plcollection", $collectiondata);
			//end saving of collection table
			$date = $this->ci->auth->localdate();
			//get CMCtransID
			if($this->ci->cash->checkOpenTrans($this->ci->auth->branch_id()) == false){
				$trans = $this->startTransaction($this->ci->auth->branch_id(), $date);
			}else{
				$branch = $this->ci->auth->branch_id();
				
				$trans = $this->getTransaction($date, $branch);
			}
			
			//save to transaction table
			/*$data = array("amount_in"=>$_POST['amountwithdrawn'],
							"transType"=>$_POST['transtype'],
							"paymentType"=>$_POST['paymentType'],
							"Particulars"=>$_POST['particular'],
							"referenceNo"=>$_POST['reference'],
				...
				foreach($_POST['amountdue'] as $loanID=>$sched){
					...
					$pntotal = 0;
						...
						foreach($sched as $sid=>$adue){
							...
							$schedule = array("Paid"=>$adue,
											"DatePaid"=>$this->ci->auth->localtime());
					...
					//add to OR Details 
					//LOANID should be PN number
					/*
					$ordetail[] = array("BankTransID"=>$banktransID,
											"description"=>$loanID,
											"value"=>$pntotal);*/	
